'''
This program's function is to test the email output in a way that does not include the multiprocessing problem.

Using the googlesearch module made by Mario Vilas at https://breakingcode.wordpress.com/
'''

import sys, requests, bs4, os, re, pprint, multiprocessing, openpyxl, pprint
from openpyxl.utils import get_column_letter  # getting the get_column_letter function
...
for data in search(string, stop=stop_number):  # for each piece of data in the search that stops at 20
    url_list.append(data)  # place the data into a que

print('Using this list of urls:')

pprint.pprint(url_list)

for item in url_list:

    print('Using: ' + str(item))
